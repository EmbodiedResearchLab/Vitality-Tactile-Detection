function [output_array,subject_quit, new_threshold] = Channel_Dynamic_Thresholding(windowPtr,threshold)
%{
...
stim(stim_trials) = struct('trial',[],'time',[],'delay_time',[],'stimulus',[],'response',[]);
...
for (i = 1:total_trials)
	...
	if trial_validity == is_real
		...
		[s, keyCode, ~] = KbWait(-3, 2, GetSecs()+1);
		WaitSecs(t - s);
        key = find(keyCode);
        %% dynamic thresholding
			...
			if stimulus == threshold_changing
                ...
                % If participant responds "yes" and stimulus intensities
                % are not 0 or 1.
				if (keyCode(30)) && (stimulus ~= intensity(1) && stimulus~= intensity(3))
    ...
    stim(i).response = KbName(find(keyCode));
